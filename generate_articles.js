        ...
        <article class="container article-container">
            ...
            <div class="article-content">
                ...
                <div style="margin-top: 50px; text-align: center;">
                    <a href="recommend.html" class="btn btn-primary" style="padding: 15px 40px; font-size: 1.2rem; border-radius: var(--radius-full); background-color: #1a1025; color: #fff; box-shadow: 0 10px 20px rgba(0,0,0,0.1);">查看柳如烟精选机场推荐</a>
                </div>
...
for (let i = 0; i < titles.length; i++) {
    ...
    const filename = filenames[i] + ".html";
    let short_title = title.split("｜")[0].split("？")[0].split("：")[0];

    let links_html = "";
    for (let j = 0; j < titles.length; j++) {
        if (i !== j) {
            links_html += `<li><a href="${filenames[j]}.html">${titles[j]}</a></li>\n                        `;
        }
    }

    let content = template.replace(/{title}/g, title).replace(/{short_title}/g, short_title).replace(/{internal_links}/g, links_html);
    fs.writeFileSync(filename, content, 'utf8');
}
console.log("Generated 12 articles.");
