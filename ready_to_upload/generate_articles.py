...
for i, title in enumerate(titles):
    ...
    short_title = title.split("｜")[0].split("？")[0].split("：")[0]
    
    # Generate random internal links
    links_html = ""
    for j, other_title in enumerate(titles):
        if i != j:
            links_html += f'<li><a href="{filenames[j]}.html">{other_title}</a></li>\\n                        '
    
    content = template.format(title=title, short_title=short_title, internal_links=links_html)
    with open(filename, "w", encoding="utf-8") as f:
        f.write(content)

print("Generated 12 articles.")
